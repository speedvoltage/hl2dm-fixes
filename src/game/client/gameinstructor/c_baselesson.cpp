...
CUtlSymbolTable CGameInstructorSymbol::m_SymbolTable;
...
{
    if (input->EnableJoystickMode() || input->IsSteamControllerActive())
    {
...
{
    if (!GetGameInstructor().IsEnabled())
        return;
    ...
    if (m_bOnlyKeyboard && (input->EnableJoystickMode() || input->IsSteamControllerActive()))
        return;

    if (m_bOnlyGamepad && !input->EnableJoystickMode() && !input->IsSteamControllerActive())
        return;
...
void CScriptedIconLesson::InitElementsFromKeys(CUtlVector<LessonElement_t> *pLessonElements, KeyValues *pKey)
{
    ...
    for (pSubKey = pKey->GetFirstSubKey(); pSubKey; pSubKey = pSubKey->GetNextKey())
    {
        ...
        Q_strncpy(szSubKeyName, pSubKey->GetName(), sizeof(szSubKeyName));
        ...
        Q_strncpy(szSubKeyName, pSubKey->GetString(), sizeof(szSubKeyName));
...
bool CScriptedIconLesson::ProcessElement(IGameEvent *event, const LessonElement_t *pLessonElement, bool bInFailedScope)
{
        ...
        if (pLessonElement->iParamVarIndex < LESSON_VARIABLE_TOTAL)
        {
            ...
            case FIELD_STRING:
                eventParam_float = static_cast<float>(
                    Q_atof((LESSON_VARIABLE_GET_FROM_OFFSET(CGameInstructorSymbol, pInfo->iOffset)).String()));
                break;
            case FIELD_EHANDLE:
            case FIELD_FUNCTION:
    ...
    case FIELD_INTEGER:
        if (pLessonElement->iParamVarIndex < LESSON_VARIABLE_TOTAL)
        {
            ...
            case FIELD_STRING:
                eventParam_float =
                    Q_atoi((LESSON_VARIABLE_GET_FROM_OFFSET(CGameInstructorSymbol, pInfo->iOffset)).String());
                break;
            case FIELD_EHANDLE:
            case FIELD_FUNCTION:
                DevWarning("Can't use this variable type with this parameter type in lesson script.\n");
    ...
    case FIELD_STRING:
        ...
        else
        {
            ...
            {
                Q_strncpy(eventParam_string, pchEventString, sizeof(eventParam_string));
            }
            else if (pLessonElement->bOptionalParam)
            {
                // We don't want to interpret this and not finding the param is still ok
                                               ...
                                               C_BaseEntity *pParam, const char *pchParam)
{
    ...
    case LESSON_ACTION_GET_PLAYER_DISPLAY_NAME: {
        ...
        C_BasePlayer *pPlayer = ToBasePlayer(pVar);
        if (!pPlayer)
            return false;

        *pString = pPlayer->GetPlayerName();
    ...
    case LESSON_ACTION_GET_WEAPON_IN_SLOT: {
        ...
        m_hEntity1 = NULL;
        for (int i = 0; i < pBaseCombatCharacter->WeaponCount(); ++i)
        {
            C_BaseCombatWeapon *pWeapon = pBaseCombatCharacter->GetWeapon(i);
            if (pWeapon && pWeapon->GetSlot() == nTemp)
            {
                m_hEntity1 = pWeapon;
                break;
            }
    ...
    case LESSON_ACTION_CLIP_PERCENTAGE_LESS_THAN: {
        ...
        if (pBaseCombatWeapon->UsesClipsForAmmo1() && pBaseCombatWeapon->GetMaxClip1() > 0)
        {
