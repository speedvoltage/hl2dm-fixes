...
void CLocatorTarget::SetCaptionColor(const char *pszCaptionColor)
{
    ...
    if (colorValues.Count() == 3)
    {
        ...
        m_captionColor.SetColor(clamp(r, 0, 255), clamp(g, 0, 255), clamp(b, 0, 255), 255);
    }
    else
    {
...
void CLocatorTarget::SetBinding(const char *pszBinding)
{
    if (!pszBinding)
        pszBinding = "";
    if (Q_strcmp(m_szBinding.String(), pszBinding) == 0 && gpGlobals->curtime < m_flNextBindingUpdate)
        return;

    m_flNextBindingUpdate = gpGlobals->curtime + 0.25f;
    m_szBinding = pszBinding;
    m_pIcon_onscreen = NULL;
    m_pIcon_offscreen = NULL;
    m_iBindingChoicesCount = 0;

    CSplitString bindings(pszBinding, ";");
    for (int i = 0; i < bindings.Count() && m_iBindingChoicesCount < MAX_LOCATOR_BINDINGS_SHOWN; ++i)
    {
        const char *pchBinding = engine->Key_LookupBindingExact(bindings[i]);
        if (!pchBinding || !pchBinding[0])
            continue;
        Q_strncpy(m_szBindingChoices[m_iBindingChoicesCount], pchBinding,
                  sizeof(m_szBindingChoices[m_iBindingChoicesCount]));
        m_iBindChoicesOriginalToken[m_iBindingChoicesCount] = i;
...
const char *CLocatorTarget::UseBindingImage(char *pchIconTextureName, size_t bufSize)
{
    ...
    const char *pchBinding = m_szBindingChoices[m_iBindingTick % m_iBindingChoicesCount];
...
class CLocatorPanel : public vgui::EditablePanel
{
  ...
  private:
    CPanelAnimationVar(vgui::HFont, m_hCaptionFont, "caption_font", "InstructorTitle");
    CPanelAnimationVar(vgui::HFont, m_hCaptionGlowFont, "caption_glow_font", "InstructorTitleGlow");


    CPanelAnimationVar(vgui::HFont, m_hKeysFont, "keys_font", "InstructorKeyBindings");
...
{
    if (s_pLocatorPanel)
        s_pLocatorPanel->DeactivateAllTargets();
}
...
{
    Assert(s_pLocatorPanel == NULL);
    SetScheme(vgui::scheme()->LoadSchemeFromFile("resource/GameInstructorScheme.res", "GameInstructorScheme"));
...
CLocatorPanel::~CLocatorPanel(void)
{
    ...
    int textures[] = {m_textureID_ArrowRight, m_textureID_ArrowLeft, m_textureID_ArrowUp,
                      m_textureID_ArrowDown, m_textureID_SimpleArrow};
    for (int i = 0; i < ARRAYSIZE(textures); ++i)
    {
        if (textures[i] != -1)
            vgui::surface()->DestroyTextureID(textures[i]);
    }
...
{
    if (!GetGameInstructor().IsEnabled() || !engine->IsInGame() ||
        enginevgui->IsGameUIVisible() || engine->IsDrawingLoadingImage())
        return;
    ...
    int iTolerance = 1.25 * (ScreenHeight() * ICON_SIZE);
...
bool CLocatorPanel::ValidateTargetTextures(CLocatorTarget *pTarget)
{
    ...
    if (!pTarget->m_pIcon_onscreen || !pTarget->m_pIcon_offscreen || (bUsesBinding && bBindingTick))
    {
        ...
        else
        {
            ...
            Q_strncpy(szIconTextureName, pTarget->GetOnscreenIconTextureName(), sizeof(szIconTextureName));
        ...
        else
        {
            ...
            Q_strncpy(szIconTextureName, pTarget->GetOffscreenIconTextureName(), sizeof(szIconTextureName));
...
void CLocatorPanel::ComputeTargetIconPosition(CLocatorTarget *pTarget, bool bSetPosition)
{
    ...
    if (bSetPosition)
    {
        int tall = ScreenHeight() * ICON_SIZE;
                                          ...
                                          bool bPointer)
{
    ...
    color32 rgbaBackgroundBytes;
    UTIL_StringToColor32(&rgbaBackgroundBytes, locator_background_color.GetString());
    Color rgbaBackground(rgbaBackgroundBytes.r, rgbaBackgroundBytes.g, rgbaBackgroundBytes.b, rgbaBackgroundBytes.a);
    rgbaBackground[3] *= fAlpha;

    color32 rgbaBorderBytes;
    UTIL_StringToColor32(&rgbaBorderBytes, locator_background_border_color.GetString());
    Color rgbaBorder(rgbaBorderBytes.r, rgbaBorderBytes.g, rgbaBorderBytes.b, rgbaBorderBytes.a);
    rgbaBorder[3] *= fAlpha;

    vgui::surface()->DrawSetColor(rgbaBorder);
    vgui::surface()->DrawFilledRect(nPosX, nPosY, nPosX + nBackgroundWide, nPosY + nBackgroundTall);
    int border = clamp(locator_background_border_thickness.GetInt(), 0,
                       MAX(0, MIN(nBackgroundWide, nBackgroundTall) / 2));
    vgui::surface()->DrawSetColor(rgbaBackground);
    vgui::surface()->DrawFilledRect(nPosX + border, nPosY + border,
                                  nPosX + nBackgroundWide - border, nPosY + nBackgroundTall - border);
}
...
void CLocatorPanel::DrawStaticIcon(CLocatorTarget *pTarget)
{
    ...
    int iconTall = ScreenHeight() * ICON_SIZE;
    ...
    // Draw the arrow.
    int iArrowSize = ScreenHeight() * ICON_SIZE; // Always square width
...
void CLocatorPanel::DrawTargetCaption(CLocatorTarget *pTarget, int x, int y, bool bDrawMultiline)
{
    ...
    if (locator_text_glow.GetBool())
    {
        ...
        color32 glowColorBytes;
    UTIL_StringToColor32(&glowColorBytes, locator_text_glow_color.GetString());
    Color glowColor(glowColorBytes.r, glowColorBytes.g, glowColorBytes.b, glowColorBytes.a);
...
void CLocatorPanel::DrawBindingName(CLocatorTarget *pTarget, const char *pchBindingName, int x, int y, bool bController)
{
    if (!bController && !IsConsole())
    {
        ...
        char szBinding[256];
        Q_strncpy(szBinding, pchBindingName ? pchBindingName : "", sizeof(szBinding));
    ...
    else
    {
        ...
        int iLargeIconShift = MAX(0, iWidth - (ScreenHeight() * ICON_SIZE + ICON_GAP + ICON_GAP));
...
void CLocatorPanel::DrawIndicatorArrow(int x, int y, int iconWide, int iconTall, int textWidth, int direction)
{
    ...
    // tall = wide = ScreenHeight() * ICON_SIZE;
